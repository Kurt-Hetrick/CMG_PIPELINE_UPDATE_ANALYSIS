...
# decompress the target vcf file into the temporary sub-folder
	...
	zcat $CORE_PATH/$PROJECT_SAMPLE/SNV/RELEASE/FILTERED_ON_BAIT/$SM_TAG"_MS_OnBait_SNV.vcf.gz" \
	>| $CORE_PATH/$PROJECT_SAMPLE/TEMP/$SM_TAG"_MS_CONCORDANCE"/$SM_TAG"_MS_OnBait_SNV.vcf"
...
CMD=$CMD' '$CORE_PATH'/'$PROJECT_SAMPLE'/TEMP/'$SM_TAG'_MS_CONCORDANCE/'$SM_TAG'_MS_OnBait_SNV.vcf'
CMD=$CMD' '$FINAL_REPORT
CMD=$CMD' '$BAIT_BED
CMD=$CMD' '$VERACODE_CSV
CMD=$CMD' '$CORE_PATH'/'$PROJECT_SAMPLE'/REPORTS/CONCORDANCE_MS_BAIT/'
