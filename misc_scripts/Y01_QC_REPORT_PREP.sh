...
#########################################################################
##### THIS SECTION IS ADDED FOR ON BAIT CONCORDANCE AND IS JUST DEV #####
#########################################################################
...
##########################################################################
	...
	if [ -f $CORE_PATH/$PROJECT_SAMPLE/REPORTS/CONCORDANCE_MS_BAIT/$SM_TAG"_concordance.csv" ];
	then
		awk 1 $CORE_PATH/$PROJECT_SAMPLE/REPORTS/CONCORDANCE_MS_BAIT/$SM_TAG"_concordance.csv" \
		| awk 'BEGIN {FS=",";OFS="\t"} NR>1 \
		{print $5,$6,$7,$2,$3,$4,$8,$9,$10}' \
		| $DATAMASH_DIR/datamash transpose \
		>> $CORE_PATH/$PROJECT_MS/TEMP/QC_REPORT_PREP_$PREFIX/$SM_TAG".QC_REPORT_TEMP.txt"
	else
